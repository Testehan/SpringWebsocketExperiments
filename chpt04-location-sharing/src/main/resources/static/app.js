document.addEventListener("DOMContentLoaded", function () {
    ...
    function onConnect() {
        ...
        stompClient.subscribe(destination, onMessageReceived);
         ...
         getCurrentLocation().then((location) => {
            ...
            receiverMarker = L.marker([location.latitude, location.longitude]).addTo(map);
            receiverMarker.bindPopup('initial dummy Location').update();
        });
        ...
        }, 20000); // 60-second interval (adjust for your needs)
    ...
    function onMessageReceived(message) {
        ...
        console.log("recieved a location message: " + JSON.stringify(location));
        updateMap(location);
    }
    ...
    function createMarker(location) {
        const marker = L.marker([location.latitude, location.longitude])
            .addTo(map)
            .bindPopup(location.sender + ' Location');
        return marker;
    }

    function updateMarker(marker, location) {
        marker.setLatLng([location.latitude, location.longitude]);
        marker.bindPopup(location.sender + ' Location').update();
    }
